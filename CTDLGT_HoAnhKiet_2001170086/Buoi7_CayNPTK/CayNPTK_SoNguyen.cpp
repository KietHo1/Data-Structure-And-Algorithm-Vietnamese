...
int DeleteTree1(TNode* &root, ItemType &X)
{
	...
	printf("\nGia tri cay nhi phan tim kiem vua xoa la: ");
	while (root != NULL)
	{
		X = root->Info;
		printf("%4d", X);
...
{ //Ham dem so nut la hien co cua cay
	if (!root) return 0;
...
{	//Nút có khoảng cách về giá trị gần nhất với phần tử x trong cây
	if (!root)
		return -1;
	int min = root->Info;
...
{	//Nút có khoảng cách về giá trị xa nhất với phần tử x trong cây
	if (!root)
		return -1;
	TNode* minLeft = root;
